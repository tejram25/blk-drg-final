...
| `flow_diagrams.pptx` | The same two stories as four slides, for the stakeholder review |

The deck is generated by `build-pptx.js` (`node build-pptx.js`), which also
writes `preview.html` and fails on overlapping boxes, off-slide shapes, or
implementation detail in a label — the same checks `build.js` runs. Its shapes
are native PowerPoint shapes, so anyone can edit them without this repo.
