/**
 * Detailed animated components (solar panel, wind turbine, robot arm, siren,
 * conveyor, EV charger, transformer, …), ported 1:1 from the Angular desktop
 * editor (`frontend/src/app/features/editor/animated-shapes.ts`) so the mobile
 * SVG canvas and the web GoJS canvas render the same premium glyphs — not plain
 * boxes — for the sample diagrams.
 *
 * Each symbol is a tree of SVG parts; moving parts carry a `class` naming the
 * motion. `bakeParts(shape, phase)` returns a new part tree with that motion
 * baked in at a given phase (0..1 over the loop): rotation about the correct
 * pivot, opacity pulses, dash marching, light cycling, level rise, etc. The
 * native canvas maps the baked tree to react-native-svg elements; the web canvas
 * serialises it to an SVG data URI for a go.Picture.
 */
...
/** Every animated-component shape name (for isAnimShape checks + convertLegacy). */
export const animShapeNames = new Set(Object.keys(ANIMATED_SYMBOLS));
export const isDetailedAnim = (shape?: string): boolean => !!shape && animShapeNames.has(shape);

/** Frames per animation loop (the editor flips through them at ~20 fps). */
export const ANIM_FRAME_COUNT = 48;

/** Rotation pivots for spinning part-groups, in order of appearance. */
...
/**
 * Return a new part tree with the class-driven motion baked in at `phase` (0..1).
 * Animated parts get a wrapping `g` transform and/or modified opacity/dashoffset,
 * exactly matching the Angular `partsToSvgFrame` so both platforms look identical.
 */
export function bakeParts(shape: string, phase: number): Part[] {
  const def = ANIMATED_SYMBOLS[shape];
  if (!def) return [];
  const pivots = [...(SPIN_PIVOTS[shape] ?? [])];
  const osc = Math.sin(2 * Math.PI * phase);
  const blink = (shift = 0) => 0.3 + 0.7 * (0.5 + 0.5 * Math.sin(2 * Math.PI * (phase * 2 + shift)));
  ...
  const bake = (p: Part): Part => {
    ...
    // they stay visible on the app's light canvas.
    ...
    const baked: Part = { tag: p.tag, attrs, children: p.children?.map(bake) };
    return wrap ? { tag: 'g', attrs: { transform: wrap }, children: [baked] } : baked;
  };
  return def.parts.map(bake);
}

/** Serialize a baked part tree to an SVG fragment string (web data URIs). */
function serialize(p: Part): string {
  const a = Object.entries(p.attrs).map(([k, v]) => `${k}="${v}"`).join(' ');
  return p.children?.length
    ? `<${p.tag} ${a}>${p.children.map(serialize).join('')}</${p.tag}>`
    : `<${p.tag} ${a}/>`;
}

/** A full SVG data URI for one animation frame (used by the web GoJS Picture). */
export function animFrameUri(shape: string, phase: number): string {
  const def = ANIMATED_SYMBOLS[shape];
  if (!def) return '';
  const pad = 3;
  const w = def.width + pad * 2;
  const h = def.height + pad * 2;
  const inner = bakeParts(shape, phase).map(serialize).join('');
  const svg =
